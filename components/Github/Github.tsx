    ...
    <div className="fixed inset-0 z-20">
      ...
      <div id="github-ss" ref={githubRef} className="relative top-[-10px] w-full h-full flex items-center justify-center bg-transparent">
        {!loading && (
            ...
            >
            <div className="flex items-center justify-center gap-2 sm:gap-4 px-3 sm:px-10 mb-2">
              <div className="">
                <Image
                  ...
                  width={100}
                  height={100}
                  className="rounded-full size-10 sm:size-12 -translate-y-1 object-cover"
                />
              </div>
              <h1 className="font-modernbold leading-tight text-lg sm:text-2xl md:text-3xl max-w-2xl py-1">
                {userStats.Repositories ? username : "User not found"}{" "}
                {userStats.Repositories && "'s Github."}
              </h1>
            </div>
            <div className="grid grid-cols-4 grid-rows-8 gap-1.5 w-full max-w-[1500px] mx-auto sm:grid-cols-6 sm:grid-rows-6 sm:gap-2 md:grid-cols-8 md:grid-rows-5 md:gap-2.5 max-[1024px]:grid-cols-5 max-[1024px]:grid-rows-6 max-[1024px]:h-[800px] max-[1024px]:overflow-y-auto lg:grid-cols-10 lg:grid-rows-4 lg:gap-3 lg:h-[600px] max-sm:min-h-[100vh]">
              <LongestStreak
                streak={userStats["Longest Streak"] || 0}
                start={userStats["Longest Streak Start"] || ""}
                end={userStats["Longest Streak End"] || ""}
                classname="p-1.5 sm:p-2 md:p-3 max-[1024px]:p-2.5 col-start-1 col-span-2 row-start-1 row-span-3 sm:col-start-1 sm:col-span-3 sm:row-start-1 sm:row-span-2 md:col-start-1 md:col-span-2 md:row-start-1 md:row-span-3 max-[1024px]:col-start-1 max-[1024px]:col-span-2 max-[1024px]:row-start-1 max-[1024px]:row-span-2 lg:col-start-1 lg:col-span-2 lg:row-start-1 lg:row-span-3 overflow-hidden min-w-0 min-h-0"
              />
              <CurrentStreak
                streak={userStats["Current Streak"] || 0}
                start={userStats["Current Streak Start"] || ""}
                end={userStats["Current Streak End"] || ""}
                classname="p-1.5 sm:p-2 md:p-3 max-[1024px]:p-2.5 col-start-3 col-span-2 row-start-1 row-span-3 sm:col-start-4 sm:col-span-3 sm:row-start-1 sm:row-span-2 md:col-start-3 md:col-span-2 md:row-start-1 md:row-span-2 max-[1024px]:col-start-3 max-[1024px]:col-span-2 max-[1024px]:row-start-1 max-[1024px]:row-span-2 lg:col-start-3 lg:col-span-2 lg:row-start-1 lg:row-span-2 overflow-hidden min-w-0 min-h-0"
              />
              <Followers
              ...
              <Achievements
                userStats={userStats}
                classname="p-1.5 sm:p-2 md:p-3 max-[1024px]:p-2.5 col-start-4 col-span-1 row-start-6 row-span-2 sm:col-start-3 sm:col-span-2 sm:row-start-5 sm:row-span-1 md:col-start-8 md:col-span-3 md:row-start-3 md:row-span-1 max-[1024px]:col-start-5 max-[1024px]:col-span-1 max-[1024px]:row-start-3 max-[1024px]:row-span-1 lg:col-start-8 lg:col-span-3 lg:row-start-3 lg:row-span-1 hidden sm:block overflow-hidden min-w-0 min-h-0"
              />
              <Languages
                userStats={userStats}
                classname="p-1.5 sm:p-2 md:p-3 max-[1024px]:p-2.5 col-start-1 col-span-2 row-start-8 row-span-1 sm:col-start-5 sm:col-span-2 sm:row-start-5 sm:row-span-1 md:col-start-3 md:col-span-2 md:row-start-3 md:row-span-2 max-[1024px]:col-start-1 max-[1024px]:col-span-2 max-[1024px]:row-start-4 max-[1024px]:row-span-2 lg:col-start-3 lg:col-span-2 lg:row-start-3 lg:row-span-2 overflow-hidden min-w-0 min-h-0"
              />
              <TimeAnalysis
                userStats={userStats}
                classname="p-1.5 sm:p-2 md:p-3 max-[1024px]:p-2.5 col-start-3 col-span-2 row-start-8 row-span-1 sm:col-start-1 sm:col-span-3 sm:row-start-6 sm:row-span-1 md:col-start-9 md:col-span-2 md:row-start-1 md:row-span-2 max-[1024px]:col-start-3 max-[1024px]:col-span-2 max-[1024px]:row-start-4 max-[1024px]:row-span-2 lg:col-start-9 lg:col-span-2 lg:row-start-1 lg:row-span-2 overflow-hidden min-w-0 min-h-0"
              />
            </div>
            <div className="px-3 sm:px-5 mt-2 w-full max-w-2xl flex mx-auto">
              <h1 className="max-w-2xl w-full font-modernbold text-lg sm:text-xl md:text-2xl mb-2">
                Contribution Graph:
              </h1>
            </div>
            <div className="px-3 sm:px-5 rounded-2xl w-full mx-auto flex flex-col relative overflow-auto">
              <div className="relative max-w-2xl mx-auto">
                <div
                  className="bg-zinc-800/20 backdrop-blur-2xl border border-zinc-200/10 backdrop-saturate-200 p-2 sm:p-3 rounded-2xl mx-auto overflow-auto max-w-2xl opacity-95 hover:opacity-100 z-[9999] cursor-pointer"
